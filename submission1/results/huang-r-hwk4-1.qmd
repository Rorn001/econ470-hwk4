...
with open('hwk4_Q1-9.pkl', 'rb') as f:
   q1_fig, q2_fig, q3_rate_fig, q3_payment_fig, q4_fig, q5_fig, q5_table, q7_fig, q7_fig_2, q8_fig, q9_fig_1, q9_fig_2 = pickle.load(f)
...
There might be too few plan per county, as on average there are less than 10 plans per county.
...
The benchmark payment has increased over time untill 2015, from 810 to 830, and dropped to 780 in 2015. The value of rebates has decreases over time consistently.  
...
Over time, MA plans become more popular. This is negatively correlated with the benchmark payments. 
...
The standard errors of the treatment effect decrease as the bandwidth increases. Except for the very narrow bandwidth in each threshold values, the significance level is robust to the choice of bandwidths. The magnitude of the treatment effect slightly changes with the bandwidth, but the direction of the effect is more consistent across different bandwidths. 
...
In both cases, we see that plans just above the threshold values have more HMOs and Part Ds than plans just below the threshold values. This suggests that contracts of these two plan types may be manipulating the raw scores to jump over the threshold values to gain more market share.
